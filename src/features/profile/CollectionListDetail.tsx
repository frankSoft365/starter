...
export default function CollectionListDetail() {
  ...
  const {
    ...
    status,
    error,
  } = useCollectionListArticlesQuery(listId);
          ...
          <CollectionInfo
            ...
            onConfirmRemove={() => {
              batchRemoveMutation.mutate(
                { listId, articleIds: Array.from(selected) },
                {
                  onSuccess: () => {
            ...
            onDelete={() =>
              navigate({ to: listRoute.to, params: { userId: user?.id ?? "" } })
            }
          />
        )}
      </div>
