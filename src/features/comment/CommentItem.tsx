  ...
  return (
    <li className="list-row">
      ...
      <div className="w-min-0 flex-1">
        <span className={`flex items-center ${headingSize}`}>
          <UserHoverLink userId={userId} linkClassName="link link-hover">
            <span>
              <strong>{username}</strong>
            </span>
          </UserHoverLink>
          {isAuthor && (
            <div className="badge badge-xs badge-primary mx-1">
              {t("badge.author")}
            </div>
          )}
          {replyToUsername ? (
            <span className={`mx-1 ${headingSize}`}>
              <em> {t("comment.replyTo")} </em>
              <span className="text-blue-500">{replyToUsername}</span>
            </span>
          ) : null}
        </span>
        ...
        <div className="flex flex-row items-center justify-start">
          ...
          <button
            type="button"
            onClick={onReply}
            className={`btn btn-link text-black ${actionSize}`}
          >
            {t("btn.reply")}
          </button>
        </div>
