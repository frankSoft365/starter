...
const unreadCountFieldMap: Record<NotificationListType, 'replyCount' | 'likeCount' | 'followCount'> = {
    reply: 'replyCount',
    like: 'likeCount',
    follow: 'followCount',
};

export default function NotificationsTab({ children }: { children: React.ReactNode }) {
    const { t } = useTranslation();
    const location = useLocation();
    const navigate = useNavigate();
    const unreadCount = useAtomValue(unreadCountAtom);

    const notificationTabMap: { name: string; path: string; type: NotificationListType }[] = [
        { name: t('notification.tab.reply'), path: notificationsIndexRoute.to, type: 'reply' },
        { name: t('notification.tab.like'), path: notificationsLikeRoute.to, type: 'like' },
        { name: t('notification.tab.follow'), path: notificationsFollowRoute.to, type: 'follow' },
    ]
        ...
        <div className="w-full md:w-3xl">
            ...
            <ul className="menu bg-base-100 menu-horizontal shadow-xs">
                {notificationTabMap.map(item => {
                    const count = Number(unreadCount[unreadCountFieldMap[item.type]]) || 0;
                    return (
                        <li key={item.path}>
                            <a
                            ...
                            >
                                {item.name}
                                {count > 0 && <span className="badge badge-xs badge-primary">{count}</span>}
                            </a>
                        </li>
                    );
                })}
