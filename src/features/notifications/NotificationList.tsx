...
export default function NotificationList({
  ...
  useEffect(() => {
    return () => {
      if (
        latestSeenId.current !== null &&
        latestSeenId.current !== lastMarkedId.current
      ) {
  ...
  const notificationItems =
    data?.pages.flatMap((page) => page.items as NotificationItem[]) ?? [];
        ...
        (notificationItems.length > 0 ? (
          <ul className="list space-y-3">
            {notificationItems.map((notification, index) => {
              const actorUsername =
                notification.actorUsername || t("profile.unknownUsername");
              ...
              return (
                <div key={notification.id}>
                  {index !== 0 && notification.id === watermarkRef.current && (
                    <div className="divider">{t("notification.watermark")}</div>
                  )}
                  <li
                    className={`list-row pl-0 ${notificationUrl ? "cursor-pointer" : ""}`}
                      ...
                      {isReplyNotification(notification) &&
                        ...
                        notification.parentComment.parentId && (
                          <div className="text-xs bg-base-300 p-2 opacity-70 mb-1">
                            ...
                            <span>
                              {notification.parentComment.parentId !==
                                notification.rootComment.id &&
                                `${t("notification.replyTo")} ${notification.parentComment.replyToUsername} : `}
                            </span>
            ...
            {!hasNextPage && (
              <li className="list-row h-24 text-center p-5">
